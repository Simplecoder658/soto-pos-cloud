...
export default function App() {
  ...
  const filteredMenu = activeTab === 'Semua' ? menu : menu.filter(item => item.category === activeTab);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      alert("PESANAN BERHASIL! Stok diperbarui.");
      await initApp(); 
    } catch (e) {
      alert("Gagal Sinkron!");
    } finally {
  ...
  if (isLoading) return <div className="h-screen flex flex-col items-center justify-center font-black text-amber-500 animate-pulse uppercase tracking-widest text-center">Menghubungkan ke Cloud...</div>;
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm text-center border-4 border-amber-500 shadow-2xl">
            <div className="flex justify-between items-center mb-6 text-slate-400 font-black text-[10px] uppercase tracking-widest"><span>Bayar Via QRIS</span><button onClick={() => setShowQRModal(false)}><X size={20}/></button></div>
          ...
          <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {filteredMenu.map(m => {
                  ...
                  return (
                    ...
                    }} className={`bg-white p-4 rounded-[2.5rem] border relative transition-all text-center group ${isOutOfStock ? 'opacity-40 grayscale cursor-not-allowed' : 'hover:shadow-xl hover:border-amber-200 cursor-pointer active:scale-95'}`}>
            ...
            <aside className="w-[380px] bg-white border-l p-8 flex flex-col shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 mb-6">
                {cart.length === 0 && <div className="h-full flex items-center justify-center text-slate-300 italic text-[10px] font-medium tracking-widest">BELUM ADA ORDERAN</div>}
                {cart.map(item => (
                  <div key={item.id} className="bg-slate-50 p-4 rounded-2xl flex justify-between items-center border border-slate-100 shadow-sm animate-in slide-in-from-right duration-300">
                    <div className="flex-1 min-w-0 mr-2"><p className="font-bold text-[10px] uppercase truncate">{item.name}</p><p className="text-[11px] font-black text-amber-600">Rp {(cleanPrice(item.price) * item.quantity).toLocaleString()}</p></div>
              ...
              <div className="pt-6 border-t-4 border-double border-slate-100">
                <div className="flex justify-between items-center mb-6"><span className="text-slate-400 text-[10px] font-black uppercase">Total Bayar</span><span className="text-3xl font-black italic tracking-tighter text-slate-900">Rp {cart.reduce((s, i) => s + (cleanPrice(i.price) * i.quantity), 0).toLocaleString()}</span></div>
                <button onClick={handleCheckout} disabled={isSyncing || cart.length === 0} className={`w-full py-5 rounded-2xl font-black text-xs shadow-xl transition-all ${cart.length === 0 ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-amber-500 text-white hover:bg-amber-600 active:scale-95 shadow-amber-200'}`}>{isSyncing ? 'SINKRON DATA...' : `PROSES ${paymentMethod.toUpperCase()}`}</button>
              </div>
            </aside>
          </>
        )}
...
function AdminPanel({ qris, onRefresh }) {
  const handleUpdate = async () => {
    const url = prompt("Masukkan Link URL Foto QRIS Baru:");
    if(url) { await updateQrisCloud(url); alert("QRIS Terupdate!"); onRefresh(); }
  };
  return (
    <main className="flex-1 p-10 bg-white">
      <h1 className="text-4xl font-black mb-12 italic uppercase text-slate-900 text-center tracking-tighter decoration-blue-500 decoration-8 underline underline-offset-8">Admin Control</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 max-w-4xl mx-auto">
        <button onClick={onRefresh} className="p-14 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center gap-5 hover:bg-amber-50 hover:border-amber-200 transition-all group active:scale-95"><RefreshCw size={48} className="text-amber-500 group-hover:rotate-180 transition-transform duration-700" /><span className="font-black text-xs uppercase text-slate-400 tracking-widest">Tarik Data Cloud</span></button>
        <button onClick={handleUpdate} className="p-14 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center gap-5 hover:bg-blue-50 hover:border-blue-200 transition-all group active:scale-95"><Camera size={48} className="text-blue-500 group-hover:scale-110 transition-transform" /><span className="font-black text-xs uppercase text-slate-400 tracking-widest">Update QRIS</span></button>
      </div>
    </main>
  );
}
...
function LoginScreen({ users, onLogin, onRefresh }) {
  ...
  const submitLogin = (e) => {
    e.preventDefault();
    if (attempts >= 5) { alert("Keamanan: Akses diblokir!"); return; }
  ...
  return (
    <div className="h-screen w-screen flex items-center justify-center bg-slate-100 p-4">
      <div className="bg-white p-14 rounded-[4rem] shadow-2xl w-full max-w-sm text-center border-b-[12px] border-amber-500">
        ...
        <h2 className="text-3xl font-black mb-10 uppercase text-slate-900 italic tracking-tighter text-center">Soto POS Cloud</h2>
        <form onSubmit={submitLogin} className="space-y-5">
          ...
          <button type="submit" className="w-full py-5 bg-slate-900 text-white rounded-3xl font-black uppercase shadow-xl hover:bg-black transition-all active:scale-95 tracking-widest text-[10px]">Akses Kasir</button>
        </form>
