...
export default function App() {
  ...
  if (isLoading) return <div className="h-screen flex items-center justify-center font-black text-amber-500 animate-pulse uppercase">Memuat...</div>;
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden font-sans select-none">
      ...
      {/* MODAL STRUK (Area yang akan diprint) */}
      {showReceipt && lastOrder && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div id="print-area-fixed" className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden border-t-[12px] border-amber-500">
            <div className="p-8 text-center border-b border-dashed border-slate-200">
              ...
              <p className="text-[10px] font-bold text-slate-400">Antrean #{lastOrder.no}</p>
            </div>
            
            <div className="p-8 space-y-4 font-mono">
              <div className="text-[10px] text-slate-500 font-bold flex-print">
              ...
              <div className="py-4 border-y border-dashed border-slate-200 space-y-2 text-[11px] font-bold text-slate-800">
                {lastOrder.items.map((item, idx) => (
                  <div key={idx} className="flex-print">
                    <span className="pr-4">{item.name} x{item.quantity}</span>
                    <span>{(item.price * item.quantity).toLocaleString()}</span>
                  </div>
                ))}
              </div>
              
              <div className="flex-print pt-2 font-sans text-slate-900 border-t border-slate-100">
                <span className="text-[10px] font-black uppercase">Total Bayar</span>
            ...
            {/* Tombol yang hilang saat diprint */}
            <div className="p-6 bg-slate-50 grid grid-cols-2 gap-3 no-print">
              <button onClick={() => window.print()} className="py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2"><Printer size={16}/> Print</button>
          ...
          <>
            <main className="flex-1 p-8 overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-black uppercase italic underline decoration-amber-500 decoration-4 underline-offset-4">Kasir: {currentUser.username}</h1>
                <button onClick={initApp} className="text-slate-300 hover:text-amber-500"><RefreshCw size={20}/></button>
              </div>
              <div className="flex gap-2 mb-8 overflow-x-auto pb-2 scrollbar-hide">
                {['Semua', 'Makanan', 'Minuman', 'Jajanan'].map(cat => (
                  <button key={cat} onClick={() => setActiveTab(cat)} className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === cat ? 'bg-amber-500 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-100'}`}>{cat}</button>
                ))}
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {filteredMenu.map(m => (
                  ...
                  }} className={`bg-white p-4 rounded-[2rem] border relative transition-all text-center shadow-sm ${m.stock <= 0 ? 'opacity-40 grayscale' : 'hover:shadow-xl hover:border-amber-200 cursor-pointer active:scale-95'}`}>
...
function AdminPanel({ config, onRefresh }) {
  ...
  return (
    <main className="flex-1 p-10 bg-white">
      <h1 className="text-4xl font-black mb-12 text-center uppercase tracking-tighter underline decoration-blue-500 decoration-8 underline-offset-8">Admin Control</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        <button onClick={async () => { await updateShiftCloud(config.shiftStatus === 'OPEN' ? 'CLOSED' : 'OPEN'); onRefresh(); }} className={`p-10 border-4 border-dashed rounded-[3.5rem] flex flex-col items-center gap-4 ${config.shiftStatus === 'OPEN' ? 'border-red-100 bg-red-50/10' : 'border-green-100 bg-green-50/10'}`}>
        ...
        <div className="p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center gap-4 bg-slate-50 text-center">
          ...
          <div className="flex gap-2 w-full">
            <label className="flex-1 py-4 bg-blue-500 text-white rounded-2xl font-black text-[9px] uppercase cursor-pointer">{isUpdating ? "PROSES..." : "UPLOAD FILE"}<input type="file" className="hidden" onChange={handleFileUpload} disabled={isUpdating} /></label>
