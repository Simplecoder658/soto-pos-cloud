...
export default function App() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setCart([]); setShowQRModal(false); alert("BERHASIL!"); await initApp();
        ...
        {view === 'admin' ? (
          ...
          <>
            <main className="flex-1 p-8 overflow-y-auto bg-white/50">
              ...
              {config.shiftStatus === 'CLOSED' && <div className="bg-red-500 text-white p-4 mb-6 rounded-2xl font-black text-center uppercase tracking-widest text-[10px] animate-pulse">Shift Ditutup Admin</div>}
            ...
            <aside className="w-[380px] bg-white border-l p-8 flex flex-col shadow-2xl">
              <h2 className="text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest text-center">Keranjang</h2>
              ...
              <div className="pt-6 border-t-4 border-double border-slate-100">
                <div className="flex justify-between items-center mb-6 text-slate-900 font-black italic"><span className="text-slate-400 text-[10px] uppercase">Total</span><span className="text-3xl tracking-tighter">Rp {cart.reduce((s, i) => s + (i.price * i.quantity), 0).toLocaleString()}</span></div>
                <button onClick={handleCheckout} disabled={isSyncing || cart.length === 0} className={`w-full py-5 rounded-2xl font-black text-xs shadow-xl transition-all ${cart.length === 0 ? 'bg-slate-100 text-slate-300' : 'bg-amber-500 text-white hover:bg-amber-600 shadow-amber-200 active:scale-95'}`}>{isSyncing ? 'MEMPROSES...' : `BAYAR SEKARANG`}</button>
              </div>
            </aside>
          </>
        )}
    ...
    <main className="flex-1 p-10 bg-white">
      ...
      <div className="grid grid-cols-1 gap-8 max-w-4xl mx-auto">
        <button onClick={toggleShift} className={`p-14 border-4 border-dashed rounded-[3.5rem] flex flex-col items-center gap-5 transition-all group active:scale-95 ${config.shiftStatus === 'OPEN' ? 'border-red-100 hover:bg-red-50' : 'border-green-100 hover:bg-green-50'}`}>
          {config.shiftStatus === 'OPEN' ? <Lock size={48} className="text-red-500" /> : <Unlock size={48} className="text-green-500" />}
          <span className="font-black text-[10px] uppercase tracking-[0.2em]">{config.shiftStatus === 'OPEN' ? 'Tutup Shift' : 'Buka Shift'}</span>
        </button>
        <button onClick={onRefresh} className="p-14 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center gap-5 hover:bg-amber-50 group active:scale-95">
          <RefreshCw size={48} className="text-amber-500 group-hover:rotate-180 transition-transform duration-700" /><span className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400">Sinkronisasi Cloud</span>
        </button>
      </div>
    </main>
  );
  ...
  return (
    <div className="h-screen w-screen flex items-center justify-center bg-slate-100 p-4">
      <div className="bg-white p-14 rounded-[4rem] shadow-2xl w-full max-w-sm text-center border-b-[12px] border-amber-500">
        <div className="w-20 h-20 bg-amber-500 rounded-[1.5rem] flex items-center justify-center text-white shadow-xl mx-auto mb-10 font-black text-4xl italic shadow-amber-200">S</div>
        <h2 className="text-3xl font-black mb-4 uppercase text-slate-900 italic tracking-tighter text-center">Soto POS Cloud</h2>
