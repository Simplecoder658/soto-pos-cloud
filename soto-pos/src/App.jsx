...
export default function App() {
    ...
    const content = `
      <html>
        <head>
          ...
          <style>
            body { font-family: monospace; padding: 20px; width: 300px; color: black; }
  ...
  if (isLoading) return <div className="h-screen flex items-center justify-center font-black text-amber-500 animate-pulse uppercase">Loading...</div>;
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden border-t-[12px] border-amber-500">
            <div className="p-8 text-center border-b border-dashed border-slate-200">
              <h2 className="text-2xl font-black uppercase italic tracking-tighter">Soto Cloud</h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase">Antrean #{lastOrder.no}</p>
            </div>
            <div className="p-8 space-y-4 font-mono">
              <div className="text-[10px] flex justify-between font-bold text-slate-500 uppercase">
              ...
              <div className="flex justify-between items-center pt-2 font-sans text-slate-900">
                <span className="text-[10px] font-black uppercase tracking-widest">Total</span>
                <span className="text-2xl font-black italic">Rp {lastOrder.total.toLocaleString()}</span>
              </div>
              <div className="text-center pt-8 text-[9px] uppercase font-bold tracking-[0.2em] opacity-40">-- Terima Kasih --</div>
            </div>
            <div className="p-6 bg-slate-50 grid grid-cols-2 gap-3">
              <button onClick={handleDirectPrint} className="py-4 bg-slate-900 text-white rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-lg hover:bg-black active:scale-95"><Printer size={16}/> Print</button>
              <button onClick={() => setShowReceipt(false)} className="py-4 bg-white border-2 border-slate-200 text-slate-400 rounded-2xl font-black uppercase text-[10px]">Tutup</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm border-4 border-amber-500 shadow-2xl text-center">
            ...
            <button onClick={handleCheckout} className="w-full py-5 bg-green-600 text-white rounded-2xl font-black shadow-lg uppercase">Selesai Bayar</button>
          </div>
        </div>
      )}
          ...
          <>
            <main className="flex-1 p-8 overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-black uppercase italic tracking-tighter decoration-amber-500 decoration-4 underline underline-offset-4 tracking-widest">Kasir: {currentUser.username}</h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {menu.filter(i => activeTab === 'Semua' || i.category === activeTab).map(m => (
                  <div key={m.id} onClick={() => {
                    ...
                    <div className="absolute top-4 right-4 text-[8px] font-black bg-slate-100 px-2 py-1 rounded-full uppercase tracking-widest">{m.stock <= 0 ? 'HABIS' : `STOK: ${m.stock}`}</div>
                    ...
                    <p className="text-amber-600 font-black text-[11px] italic">Rp {Number(m.price).toLocaleString()}</p>
                  </div>
                ))}
              </div>
            </main>
            <aside className="w-[350px] bg-white border-l p-8 flex flex-col shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 mb-6 scrollbar-hide">
                {cart.map(item => (
                  <div key={item.id} className="bg-slate-50 p-4 rounded-2xl flex justify-between items-center border border-slate-100 shadow-sm">
                    <div className="flex-1 min-w-0 mr-2"><p className="font-bold text-[10px] uppercase truncate">{item.name}</p><p className="text-[11px] font-black text-amber-600 italic">Rp {(item.price * item.quantity).toLocaleString()}</p></div>
              ...
              <div className="pt-6 border-t-4 border-double border-slate-100">
                <div className="flex justify-between items-center mb-6 text-slate-900 font-black italic text-3xl tracking-tighter"><span>Rp</span><span>{cart.reduce((s, i) => s + (i.price * i.quantity), 0).toLocaleString()}</span></div>
                <button onClick={handleCheckout} disabled={isSyncing || cart.length === 0} className={`w-full py-5 rounded-2xl font-black text-xs shadow-xl transition-all tracking-[0.2em] uppercase ${cart.length === 0 ? 'bg-slate-100 text-slate-300' : 'bg-amber-500 text-white shadow-amber-200 active:scale-95'}`}>{isSyncing ? 'SINKRON...' : `PROSES PEMBAYARAN`}</button>
              </div>
            </aside>
          </>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        ...
        <div className="p-10 border-4 border-dashed border-slate-100 rounded-[3.5rem] flex flex-col items-center gap-4 bg-slate-50">
          ...
          <div className="flex gap-2 w-full">
            <label className="flex-1 py-4 bg-blue-500 text-white rounded-2xl font-black text-[9px] uppercase text-center cursor-pointer tracking-widest shadow-lg shadow-blue-100">{isUpdating ? "PROSES..." : "UPLOAD FILE"}<input type="file" className="hidden" onChange={handleFileUpload} disabled={isUpdating} /></label>
            <button onClick={() => { const u = prompt("Masukkan Link Gambar QRIS:"); if(u) updateQrisCloud(u).then(onRefresh); }} className="flex-1 py-4 bg-white border-2 border-slate-200 rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-100 transition-all">PASTE LINK</button>
          </div>
        </div>
      </div>
    </main>
