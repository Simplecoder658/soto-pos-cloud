...
export default function App() {
  ...
  const handleMenuClick = (m) => {
    // Daftar soto yang punya pilihan karbo sesuai PDF
    const sotoItems = [
  ...
  const addToCart = (item, addon = "") => {
    ...
    // Penyesuaian Harga sesuai PDF: Singkong (Base), Nasi (-1rb), Lontong (-2rb)
    // Berlaku untuk menu Utama (bukan porsi adik/kalsot)
    ...
    const inCart = cart.find(x => x.itemKey === itemKey);
  ...
  const handleCheckout = async () => {
    ...
    } catch (e) { alert("Gagal Simpan ke Cloud!"); } finally { setIsSyncing(false); }
  };
  ...
  const handleCetakPDF = () => {
    ...
    lastOrder.items.forEach((item) => {
      doc.setFontSize(8).text(`${item.name.substring(0, 20)}`, 5, yPos);
      doc.text(`x${item.quantity}`, 35, yPos);
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden font-sans text-slate-900">
          ...
          <>
            <main className="flex-1 p-8 overflow-y-auto scrollbar-hide">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {menu.map(m => (
                  <div key={m.id} onClick={() => handleMenuClick(m)} className="bg-white p-4 rounded-[2.5rem] border hover:shadow-xl cursor-pointer active:scale-95 transition-all text-center shadow-sm flex flex-col items-center justify-between min-h-[180px]">
            ...
            <aside className="w-[380px] bg-white border-l p-8 flex flex-col shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 mb-6 scrollbar-hide">
                {cart.map(item => (
                  <div key={item.itemKey} className="bg-slate-50 p-5 rounded-3xl flex justify-between items-center border border-slate-100">
                    ...
                    <div className="flex items-center gap-2 font-black text-xs">
                      <span className="px-3 py-1 bg-white rounded-xl border">{item.quantity}x</span>
              ...
              <div className={`mb-6 p-5 rounded-[2.5rem] shadow-xl transition-all ${isNumberUsed ? 'bg-red-50 border-2 border-red-500' : 'bg-slate-900'}`}>
                ...
                <input type="number" value={orderNumber} onChange={(e) => setOrderNumber(e.target.value)} className={`w-full py-4 rounded-2xl text-center text-4xl font-black outline-none transition-all ${isNumberUsed ? 'bg-white text-red-600' : 'bg-slate-800 text-white'}`} placeholder="---" />
              </div>
      ...
      {/* MODAL ADD-ON (PILIHAN KARBO) */}
      {addonModal && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[3.5rem] p-10 shadow-2xl border-t-[12px] border-amber-500 text-center">
            <p className="text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">Pilih Karbohidrat</p>
            ...
            <div className="space-y-3">
              {(addonModal.name.includes("Porsi Adik") || addonModal.name.includes("Kalsot") ? ['Nasi', 'Lontong'] : ['Singkong', 'Nasi', 'Lontong']).map(opt => (
                <button key={opt} onClick={() => addToCart(addonModal, opt)} className="w-full py-5 bg-slate-50 hover:bg-amber-500 hover:text-white rounded-2xl font-black uppercase transition-all border-2 border-slate-100">
                  + {opt}
                </button>
              ))}
      ...
      {showQRModal && (
        <div className="fixed inset-0 z-[120] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm text-center border-4 border-amber-500 shadow-2xl">
            <img src={config.qris} className="w-full aspect-square object-contain mb-4 border rounded-3xl p-2 bg-white" alt="QRIS" />
            <p className="text-3xl font-black italic mb-6 text-slate-800">Rp {cart.reduce((s, i) => s + (i.price * i.quantity), 0).toLocaleString()}</p>
            <button onClick={handleCheckout} className="w-full py-5 bg-green-600 text-white rounded-2xl font-black uppercase shadow-lg">Sudah Bayar</button>
          </div>
        </div>
      )}
    ...
    <main className="flex-1 p-10 bg-white overflow-y-auto">
      ...
      <div className="max-w-md mx-auto space-y-6">
        <div className="p-8 bg-slate-50 rounded-[3rem] border-2 border-slate-100">
          ...
          <button onClick={() => { if(confirm("Reset nomor pesanan?")) resetOrders(); }} className="w-full py-4 bg-red-500 text-white rounded-2xl font-black uppercase text-[10px] shadow-lg">Reset No. Pesanan</button>
        </div>
        <button onClick={async () => { await updateShiftCloud(config.shiftStatus === 'OPEN' ? 'CLOSED' : 'OPEN'); onRefresh(); }} className="w-full p-10 border-4 border-dashed border-slate-200 rounded-[3.5rem] font-black uppercase text-slate-500">Shift: {config.shiftStatus}</button>
      </div>
    </main>
  );
}
